<?
  class ina_translator{
    ...
    function ina_translator(){
		  ...
		  #$this->hostname = 'app.dev.ark.org';
		  #$this->hostname = 'test01.ark.org';
		  #$this->hostname = 'ina2.ark.org';
			$this->hostname = 'pmt-test.ark.org';
			$this->port = 2000;
      ...
      $this->showWDDX = 0;
		...
		function setHost($host, $port=2001){
		  $this->hostname = $host;
			$this->port = $port;
			if($this->SOCKET){
	  		$this->closeSocket();					
    ...
    function sendCommand($Function, $Params){
	    ...
	    // go through Params and fix any 0 length arrays
			$this->recurseDepth = 0;
			$this->fixArrays($Params);#jay removed pass by ref
