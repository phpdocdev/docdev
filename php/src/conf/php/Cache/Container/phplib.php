...
// +----------------------------------------------------------------------+
//
// $Id: phplib.php,v 1.1.1.1 2001/05/08 21:49:59 david Exp $
...
* @version  $Id: phplib.php,v 1.1.1.1 2001/05/08 21:49:59 david Exp $
* @package  Cache
* @see      save()
*/
class Cache_Container_phplib extends Cache_Container {
    ...
    */
    ...
    function garbageCollection($maxlifetime) {
        ...
        $this->db->query( 
                        sprintf("DELETE FORM %s WHERE (expires <= %d AND expires > 0) OR changed <= (NOW() - %d)", 
                            $this->cache_table, 
                            time(),
                            $maxlifetime
                        )
